...
1. **`AmfLibrary` をシングルトン化** (`src/lib.rs`)
   ...
   - `AmfLibrary` および `DynLib` に `unsafe impl Sync` を追加
...
3. **Encoder/Decoder/ProbeContext から `_lib` フィールドを削除**
   - `src/encode.rs`, `src/decode.rs`, `src/codec_info.rs`
   - ライブラリハンドルは `static AMF_LIBRARY` が永続的に保持するため不要
   - `factory_ptr()` は `&self` メソッドとして `instance()` 経由でアクセス
   - 呼び出し側は `let lib = AmfLibrary::instance();` で取得し、以降 `lib.create_context()`, `lib.init_vulkan(context)` のように使う

4. **テストの `AMF_LOCK` を削除**
   - `tests/test_roundtrip.rs` (24 テスト), `tests/test_codec_info.rs` (2 テスト)
   - 本番コード側で排他制御が実装されたため不要になった

5. **`DynLib` に `Sync` を実装** (`src/dl.rs`)
   - 内部ハンドルは構築後に変更されず読み取りのみのため安全

### API 変更

- `AmfLibrary::load()`（戻り値 `Result<Self, Error>`）を削除
- `AmfLibrary::instance()`（戻り値 `&'static AmfLibrary`）を追加。`LazyLock` により infallible
- `AmfLibrary::factory_ptr()` の可視性を `pub(crate)` にし、戻り値を `Result<*mut AMFFactory, Error>` に変更
